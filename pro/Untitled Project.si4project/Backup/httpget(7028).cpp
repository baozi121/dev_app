...
#define FILENAME "/opt/baozi/postwrite.txt"  
#endif
   ...
   */
	...
	if(curl)
	{
	 // char *raw="devid=310023001301920001";
	  ...
	  curl_easy_setopt(curl, CURLOPT_URL, "http://101.69.255.131:9000/Dev/getDevOptsRes");
	  sprintf(p_param,"devId=%s&opt_res=%s",devid_get,param);
	  printf("p_param is %s\n",p_param);
...
int post_tcp(char * param) {  
	...
	if(curl)
	{
	  curl_easy_setopt(curl, CURLOPT_URL, "http://101.69.255.131:9000/Dev/tcpExperimentOpts");
	  sprintf(p_param,"devId=%s&opt_res=%s",devid_get,param);
	  printf("p_param is %s\n",p_param);
	  curl_easy_setopt(curl, CURLOPT_POSTFIELDSIZE, strlen(p_param ));
	  /* Now specify the POST data */
...
int HttpDownloadfile(char *filename,char *devid,char *return_filename)
{
	char url[256]={0};
	char *ptr_dot;
	char filename_local[50]={0};//最终创建文件的绝对路径
	char dfname[30]={0};//文件名字结尾去掉“ . ”
  CURL *curl;
  CURLcode res;
  ptr_dot=strstr(filename,".");
  if(ptr_dot !=NULL)
  {
	if(strlen(ptr_dot)<3)
		{
				printf("#####  ---....####\n");
			 strncpy(dfname,filename,(strlen(filename)-1));
		}
  }
  strncpy(dfname,filename,strlen(filename));
  sprintf(filename_local,"/opt/baozi/download/%s",dfname);
  printf("dfname == %s \n",dfname);
  printf("filename_local ==== %s\n",filename_local);
  memcpy(return_filename,filename_local,strlen(filename_local));
  FILE* pFile = fopen( filename_local, "wb" );
  if( pFile == NULL ){
	...
	sprintf(url,"192.168.1.220:8080/Dev/DevDownloadFile?devId=%s&fileName=%s",devid,filename);
	printf("sprintf after url = %s\n",url);
#endif
  curl_global_init(CURL_GLOBAL_DEFAULT);
