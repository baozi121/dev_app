									  ...
									  while(1)//读取文件结束判断
									   {
										...
										if((strlen(tcp_final_gets)+strlen(tcplogtxt))<sizeof(tcp_final_gets))
		 ...
		 pNode_child = pNode_root->FirstChildElement("syxopts");
		   ...
		   cmd_tmp= const_cast<char*>(szXmlVaule);
			...
			{	
				...
				if(tcp_fp!=NULL)
				fwrite(cmd_tmp, 1, strlen(cmd_tmp),tcp_fp);
			}
/*********tcp*************/
				...
				{
			   		sprintf(cmd,"%s &> /opt/baozi/tcplog.txt ",cmd_tmp);//标准输入/错误重定向到一个文件
									  printf("cmd is %s\n",cmd);
		          	int ret = pthread_create(&tcp_result_get, NULL, tcp_re_get,NULL);
				  	pthread_detach(tcp_result_get);//线程分离
									  tcp_fp=popen(cmd,"w");
				  					  tcp_start_flag=1;
                    tcplog_fd=fopen("/opt/baozi/tcplog.txt","r+");
						printf("tcp\n");
			   }
