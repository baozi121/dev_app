...
{
	char url[256]={0};
	char filename_local[30]={0};
  CURL *curl;
  CURLcode res;
  sprintf(filename_local,"/opt/baozi/download/%s",filename);
  printf("filename_local ==== %s\n",filename_local);
  FILE* pFile = fopen( "/opt/baozi/download/dev_app_pn", "wb" );
  if( pFile == NULL ){
  ...
  curl = curl_easy_init();
  if(curl) {
   curl_easy_setopt(curl,CURLOPT_URL,"192.168.1.220:8080/Dev/DevDownloadFile?devId=310023003100230001&fileName=dev_app_pn.");
    //curl_easy_setopt(curl,CURLOPT_URL,url);
