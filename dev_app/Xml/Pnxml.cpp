		    ...
		    if(strncmp(cmd_tmp,"cd ",3)==0){   
			   ...
			   // if( (strncmp(cmd_tmp,"cd ..",5)==0) || (strncmp(cmd_tmp,"cd -",4)==0) )
				...
				/*****cd正确后更新当前和上一次所在目录位置******/
							...
							sprintf(pwd_post,"current pwd == %s",p);
							post(pwd_post);
						}
			    }
				/*********cd -*************/
				else if( (strncmp(cmd_tmp,"cd -",4)==0))
						{
							...
							sprintf(pwd_post,"current pwd == %s",current_pwd);	
							post(pwd_post);
...
/*********history*************/
			else if(strncmp(cmd_tmp,"history",7)==0)
			{
				...
				int i=0;
				char buf[256] = {0};
				printf("history cmd_tmp is %s\n",cmd_tmp);
				system("history 10 &> history.txt");
				fd_his=fopen("history.txt","r+");
				if(NULL == fd_his)
					{
							perror("popen fp history error.");
							return -1;
					}
				memset(buf,0,sizeof(buf));
				fseek(fd_his,0,SEEK_SET);
				fflush(fd_his);
					while(1)
					{
							fgets(buf, sizeof(buf), fd_his);
					...
					printf("start history ping return\n");
					post(final_gets);
			}
...
#if 0
	//recive Rtmp_addr  ----cd
 if ((pNode_root = XmlDoc.FirstChild("start_video")) != NULL)
    {

        pNode_child = pNode_root->FirstChildElement("video_url");
        if (NULL == pNode_child)
        {
            return PN_ERR_INVALID_DATA;
        }
